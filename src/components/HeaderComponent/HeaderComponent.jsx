...
const HeaderComponent = ({logoutFunction, followRequests}) => {
      ...
      <div className='container-opt'>
        {
          followRequests &&
          (
            followRequests[0].status === "pending" ? 
            (
        ...
        <div className={`notifications-container ${notifications ? "show": "hidden"}`}>
          {
            ...
            (
              followRequests[0].status === "pending" ?
              (
                followRequests.map((fq, idx)=> (
                  <div key={idx} className='requester-info' >
                    <img src={fq.requester.imgProfile}/>
                    <div className='name-and-action'>
                      <span className='name'>{fq.requester.userName}</span>
                      <span>has requested to follow you</span>
                    </div>
                    <div className='buttons'>
                      <button onClick={() => handleFollowRequest(fq._id)}>Accept</button>
                      <button onClick={() => console.log(fq._id)} style={{backgroundColor:"red", color:"white"}}>Reject</button>
                    </div>
                  </div>
                ))
              )
              :
              ( 
                  <span>No follow request pending</span>
              )
            )
